...
#                 "--background", "0"
        ...
        output_states, rnd = SCP_Skeletonizer(BW_Image, round_debug,input_symbol_debug,output_symbol_debug, args.debug)

        # ===== Reconstruct SKL Image =====
        save_pathproc: str
        if not args.negative:
            save_pathproc = f'output-images/{img_name}-TR{args.threshold}-SKL-SPS.png'
        else:
            save_pathproc = f'output-images/{img_name}-TR{args.threshold}-SKL-neg-SPS.png' 
        ...
        if args.logging:
            ...
            log_maker(img_name, width,height,args.threshold, st, et, elapsed, rnd, args.negative)
